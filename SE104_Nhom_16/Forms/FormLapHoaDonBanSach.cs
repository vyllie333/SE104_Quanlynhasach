...
namespace SE104_Nhom_16.Forms
{
    public partial class FormLapHoaDonBanSach : Form
    {
                ...
                if (dem == 1)
                {
                    ...
                    foreach (ListViewItem item in this.listViewSach.Items)
                    {
                        if (item.Checked == true)
                        {
                            ...
                            else if ((SoLuongSachConLai - SoLuongTonToiThieu) >= Convert.ToInt32(numericUpDownSoLuong.Value))
                            {
        ...
        private void btnLapHoaDon_Click(object sender, EventArgs e)
        {
            ...
            int tienthua_ = 0;
            Int32.TryParse(txtTienThua.Text, out tienthua_);
            Console.WriteLine(tienthua_);
            if (txtKhachTra.Text=="0")
            {
                MessageBox.Show("KHÁCH KHÔNG TRẢ TIỀN!", "THÔNG BÁO");
            }    
            if (tienthua_ < 0 && txtKhachTra.Text != "0")

            {
                MessageBox.Show("KHÁCH TRẢ TIỀN 1 PHẦN, KHÔNG TRẢ TIỀN THỪA CHO KHÁCH!", "THÔNG BÁO");
            }    
